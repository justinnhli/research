...
def run_wsd(guess_method, activation_base=2, decay_parameter=0.05, constant_offset=0, iterations=1, num_sentences=-1,
            partition=1, outside_corpus=False, clear_network="never", context="word", whole_corpus=True):
    """
...
def get_corpus_accuracy(guess_method, corpus_utilities, clear_network="never", activation_base=2,
                        ...
                        context="word", whole_corpus=False, index_info=False):
    """
    ...
    if (guess_method == "cooc") and (whole_corpus is False):
        ...
        cooc_thresh_sem_agent = AgentCoocThreshSpreadingCorpus(context_type=context, whole_corpus=whole_corpus,
                                                               corpus_utilities=corpus_utilities,
    ...
    for iter in range(iterations):
        ...
        if "naive_semantic" in guess_method and iter > 1:
            ...
            sem_never_network = oracle_agent.sem_never_agent.clear_sem_network(oracle_agent.sem_never_network, 1)
            sem_nospread_network = oracle_agent.sem_nospread_agent.clear_sem_network(oracle_agent.sem_nospread_network,
                                                                                     1)
            timer_never = 2
...
# for i in range(1, 7):
#     print("partition", i)
#     print("Word, Original", run_wsd("cooc", iterations=1, num_sentences=5000, partition=i, context="word", whole_corpus=False))
#     print("Sense, Original", run_wsd("cooc", iterations=1, num_sentences=5000, partition=i, context="sense", whole_corpus=False))
#     print("Word, Whole", run_wsd("cooc", iterations=1, num_sentences=5000, partition=i, context="word", whole_corpus=True))
#     print("Sense, Whole", run_wsd("cooc", iterations=1, num_sentences=5000, partition=i, context="sense", whole_corpus=True))